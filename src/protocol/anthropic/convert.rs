...
use crate::protocol::anthropic::schema::{Block, Message, MessageContent, MessagesRequest, SystemPrompt, Thinking};
use crate::protocol::warning::{Decoded, Warning};
...
fn breakpoint(block: &Block) -> Option<crate::core::CacheControl> {
    block.cache_control.as_ref().map(crate::core::CacheControl::from_json)
}
...
mod tests {
    ...
    fn a_breakpoint_kind_is_kept_rather_than_normalized() {
        ...
        let Some(crate::core::CacheControl { kind }) = decoded.value.system[0].cache_control() else {
