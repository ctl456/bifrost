...
fn apply_prompt_cache_key(request: &mut CanonicalRequest) {
    ...
    if request.system.is_empty()
        || request
            .system
            .iter()
            .any(crate::core::ContentBlock::is_cache_breakpoint)
    {
        return;
    }
    let last = request.system.len() - 1;
    if let crate::core::ContentBlock::Text { cache_control, .. } = &mut request.system[last] {
        *cache_control = Some(crate::core::CacheControl::ephemeral());
    }
}
