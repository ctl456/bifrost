...
use crate::core::{ChunkGenerator, FinishReason, OutputChunk, Usage};
use serde_json::{Value, json};

use crate::protocol::openai::shared::OpenAiUsage;
use crate::protocol::sse::{DONE, SseFrame};
    ...
    pub fn render(&mut self, chunk: &OutputChunk) -> Vec<SseFrame> {
        match chunk {
            ...
            OutputChunk::ToolCall { id, name, arguments } => {
                let id = crate::core::tool_call_id(id.as_deref(), self.tool_call_index);
...
pub fn completion_body(
    meta: &crate::protocol::adapter::ResponseMeta,
    response: &crate::core::CanonicalResponse,
) -> Value {
    ...
    for block in &response.content {
        match block {
            crate::core::ContentBlock::Text { text: part, .. } => text.push_str(part),
            crate::core::ContentBlock::Reasoning { text: part } => reasoning.push_str(part),
            crate::core::ContentBlock::ToolUse { id, name, input } => tool_calls.push(json!({
                "id": id,
                "type": "function",
                "function": {
                    "name": name,
...
#[must_use]
pub fn failure_frame(error: &crate::core::Error) -> SseFrame {
...
mod tests {
    use super::*;
    use crate::core::OutputAccumulator;
    ...
    fn an_upstream_error_emits_no_frame() {
        ...
        let frames = frames_text(
            &mut renderer,
            &OutputChunk::UpstreamError {
                error: crate::core::Error::upstream("boom"),
            },
        );
        assert!(frames.is_empty(), "an error must not be preceded by a finish frame");
    }

    fn meta() -> crate::protocol::adapter::ResponseMeta {
        crate::protocol::adapter::ResponseMeta {
