...
/// How much of a key digest is kept. Sixteen hex characters identify a key without
/// being one, which is all a log line needs.
...
/// A digest of the key a turn was billed to, never the key.
///
/// The access line names the key this way because the line is read by people and
/// collected by machines, and neither of them should be holding a credential: two
/// lines that carry the same fingerprint are the same key, which is the whole
/// question an operator asks of it.
#[must_use]
pub fn key_fingerprint(api_key: &str) -> String {
    let digest = Sha256::digest(api_key.as_bytes());
    hex::encode(digest)[..KEY_FINGERPRINT_LEN].to_owned()
}
