...
impl Session {
    ...
    pub async fn open(plan: StreamPlan) -> Opening {
        ...
        let reader = tokio::spawn(pump(response, sender, Arc::clone(&backpressure)));
        ...
        loop {
            match session.step().await {
                ...
                Step::Idle => return unwritten(crate::edge::state::idle_timeout_error()),
                Step::Ended => {
                    ...
                    if let Some(error) = session.accumulator.error.clone() {
                        return unwritten(error);
                    }
                    return unwritten(crate::edge::state::empty_output_error());
                }
                Step::Failed(error) => return unwritten(error),
            }
        }
    }
            ...
            match self.step().await {
                ...
                Step::Ended => {
                    ...
                    let closing = if closing.is_empty() && self.accumulator.is_empty_output() {
                        self.renderer.failure_frames(&crate::edge::state::empty_output_error())
                    } else {
                        closing
                    };
                ...
                Step::Idle => {
                    self.done = true;
                    let frames = self.renderer.failure_frames(&crate::edge::state::idle_timeout_error());
    ...
    async fn step(&mut self) -> Step {
        ...
        match tokio::time::timeout(tick, self.incoming.recv()).await {
            ...
            Err(_elapsed) => {
                ...
                {
                    self.last_sent = Instant::now();
                    return Step::Frames(vec![crate::protocol::anthropic::response::ping_frame()]);
                }
                Step::Frames(Vec::new())
            }
        }
...
/// Hand back a status that was never spent.
fn unwritten(error: Error) -> Opening {
...
async fn pump(mut response: reqwest::Response, sender: mpsc::Sender<Pump>, backpressure: Arc<Backpressure>) {
    loop {
