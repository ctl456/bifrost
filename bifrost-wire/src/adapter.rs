...
use bifrost_config::AdapterId;
use bifrost_core::{CanonicalRequest, ConversionError};
use bifrost_fingerprint::Fingerprint;
...
pub fn adapter_for(id: &AdapterId) -> Option<Box<dyn WireAdapter>> {
    match (id.name.as_str(), id.version.as_str()) {
        ("cc", "1.53.1") => Some(Box::new(crate::cc::CcV1531::default())),
        _ => None,
    }
}
