...
`tools/smoke.sh` 会起一个构建、和真实服务对话，然后再起同一个构建的第二个部署——这次是自己发牌的——并把两份日志都读回来：每个请求留下的访问行、被拒预请求才会出现的警告、两个日志里都没有凭据、以及停止信号是否干净退出。它对发牌部署问的每一项都用一个不可能生效的凭据去问，所以这一段不花钱（拒绝在上游之前就决定了），只有 `--generate` 下那一个回合例外。两个外部检查都能证明自己有能力失败——smoke 用 `--self-test`，对齐脚本用 `CC_SELFTEST=1`。
