...
pub async fn send(edge: &Edge, request: &bifrost_wire::WireRequest) -> Result<Upstream, Error> {
...
/// A complete upstream answer: what arrived, and what it decodes to.
pub struct Complete {
    /// The body exactly as the upstream sent it. Kept as bytes rather than text so
    /// that an archived answer is the answer, not a lossy round trip through
    /// UTF-8.
    pub raw: Bytes,
    pub chunks: bifrost_core::OutputAccumulator,
}
...
pub async fn read_output(response: reqwest::Response) -> Result<Complete, Error> {
    ...
    let mut decoder = bifrost_wire::cc::CcDecoder::new();
    let mut chunks = bifrost_core::OutputAccumulator::new();
