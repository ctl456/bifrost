...
use bifrost_fingerprint::{DeviceProfile, generate_fingerprint};
...
async fn an_oversized_body_is_refused_with_413() {
    ...
    let config = Config {
        ...
        limits: bifrost_config::LimitsConfig {
            max_body_mb: 1,
            ..Default::default()
        },
        ..Config::default()
...
fn temp_dir(label: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!("bifrost-edge-{}-{label}-{unique}", std::process::id()))
}

/// The one turn in a journal, once the audit has caught up with the response.
///
/// The record is written off the request path on purpose, so a test that read the
/// journal the instant the client saw its last byte would be racing a design
/// decision rather than catching a bug.
async fn only_turn(journal: &Journal) -> Value {
    for _ in 0..200 {
    ...
    panic!("the turn was never recorded");
}

/// The handle the journal says a blob is stored under.
fn reference(captured: &Value) -> ArchiveRef {
    ArchiveRef {
        sha256: captured["sha256"].as_str().expect("sha256").to_owned(),
        bytes: captured["bytes"].as_u64().expect("bytes"),
        lines: captured["lines"].as_u64().expect("lines"),
    }
}

fn archive_config(root: &Path, api_base: &str) -> Config {
    Config {
        api_base: api_base.to_owned(),
        ...
        audit: AuditConfig {
            journal_dir: root.join("journal.jsonl"),
...
#[tokio::test]
async fn an_archived_turn_keeps_both_bodies_under_their_digests() {
    let (upstream, _) = upstream(|| (200, DELTAS.to_owned(), "application/x-ndjson")).await;
    let root = temp_dir("evidence-complete");
    let edge = serve(archive_config(&root, &upstream)).await;
    ...
    let entry = only_turn(&journal).await;
    assert_eq!(entry["kind"], json!("turn"));
    assert_eq!(entry["data"]["protocol"], json!("openai-chat"));
    assert_eq!(entry["data"]["stream"], json!(false));
    assert_eq!(entry["data"]["status"], json!(200));
    assert_eq!(entry["data"]["model"], json!("deepseek/deepseek-v4-flash"));
    assert_eq!(entry["data"]["key"], json!(key_fingerprint("user_abc123")));
    assert!(
        !entry["data"].to_string().contains("user_abc123"),
        "the journal names the key without being one: {entry}"
    );

    // The journal is enough to get the evidence back: the digest it recorded is
    // the address, so a reader can check what it finds against what was claimed.
    let store = ArchiveStore::new(root.join("archive"));
    let request = store
        .read(bifrost_audit::KIND_REQUEST, &reference(&entry["data"]["request"]))
        .expect("read the request back");
    assert_eq!(serde_json::from_slice::<Value>(&request).expect("json"), payload);
    let answer = store
        .read(
            bifrost_audit::KIND_UPSTREAM_RESPONSE,
            &reference(&entry["data"]["response"]),
        )
        .expect("read the answer back");
    assert_eq!(
        answer,
        DELTAS.as_bytes(),
        "the archive holds what arrived, not a re-render of it"
    );
}

#[tokio::test]
async fn a_streamed_turn_is_archived_as_the_upstream_sent_it() {
    let (upstream, _) = upstream(|| (200, DELTAS.to_owned(), "application/x-ndjson")).await;
    let root = temp_dir("evidence-stream");
    let edge = serve(archive_config(&root, &upstream)).await;
    ...
    let journal = Journal::new(root.join("journal.jsonl"));
    let entry = only_turn(&journal).await;
    assert_eq!(entry["data"]["stream"], json!(true));
    assert_eq!(entry["data"]["status"], json!(200));

    let store = ArchiveStore::new(root.join("archive"));
    let events = store
        .read(
            bifrost_audit::KIND_UPSTREAM_EVENT,
            &reference(&entry["data"]["response"]),
        )
        .expect("read the events back");
    assert_eq!(events, DELTAS.as_bytes());
    assert_eq!(entry["data"]["response"]["truncated"], json!(false));
}

#[tokio::test]
async fn a_deployment_that_did_not_ask_for_evidence_writes_nothing() {
    // The default configuration: the turn runs, and no file is created for it.
    let (upstream, _) = upstream(|| (200, DELTAS.to_owned(), "application/x-ndjson")).await;
    let root = temp_dir("evidence-off");
    let config = Config {
        api_base: upstream.clone(),
        audit: AuditConfig {
            journal_dir: root.join("journal.jsonl"),
    ...
    assert!(!root.join("journal.jsonl").exists(), "the mechanism is opt-in");
...
async fn a_quiet_upstream_is_reported_as_a_retryable_timeout() {
    let upstream = stalling_upstream().await;
    let root = temp_dir("timeout-nonstream");
    let config = Config {
        limits: LimitsConfig {
            nonstream_idle_ms: 100,
            ..Default::default()
        },
        ..archive_config(&root, &upstream)
    };
    let edge = serve(config).await;
    ...
    let entry = only_turn(&Journal::new(root.join("journal.jsonl"))).await;
    assert_eq!(entry["data"]["status"], json!(429));
    assert_eq!(entry["data"]["response"], Value::Null, "there was nothing to archive");
}

#[tokio::test]
async fn a_stream_that_never_committed_is_recorded_with_the_status_the_client_got() {
    let upstream = stalling_upstream().await;
    let root = temp_dir("timeout-stream");
    let config = Config {
        limits: LimitsConfig {
            stream_idle_ms: 100,
            ..Default::default()
    ...
    assert_eq!(entry["data"]["stream"], json!(true));
    assert_eq!(
        entry["data"]["response"]["bytes"],
        json!(0),
        "the archived stream is what the upstream sent, which was nothing: {entry}"
    );

    // One line, never two: the record is written either by the caller, which knows
    // the status, or by the stream as it is dropped, and never by both. The wait
    // gives a second writer time to show up rather than letting it hide.
    tokio::time::sleep(Duration::from_millis(200)).await;
    assert_eq!(journal.read_all().expect("journal").len(), 1);
}
...
async fn a_first_turn_announces_the_key_and_later_ones_do_not() {
    ...
    // The record is the identity `bifrost-fingerprint` derives for this key, in
