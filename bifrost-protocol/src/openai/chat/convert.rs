...
fn decode_tools(wire: &ChatCompletionRequest, warnings: &mut Vec<crate::warning::Warning>) -> Vec<ToolDefinition> {
    ...
    tools
        ...
        .filter_map(|(index, tool)| {
            if tool.r#type != "function" {
                warnings.push(crate::warning::Warning::new(
...
fn decode_tool_choice(wire: &ChatCompletionRequest, warnings: &mut Vec<crate::warning::Warning>) -> Option<ToolChoice> {
...
fn decode_messages(messages: &[ChatMessage], warnings: &mut Vec<crate::warning::Warning>) -> Vec<CanonicalMessage> {
...
fn user_content(index: usize, message: &ChatMessage, warnings: &mut Vec<crate::warning::Warning>) -> Vec<ContentBlock> {
            ...
            for (part_index, part) in parts.iter().enumerate() {
                ...
                match part.kind.as_str() {
                    "image_url" => match part.image_url.as_ref() {
                        ...
                        None => warnings.push(crate::warning::Warning::new(
        ...
        Some(MessageContent::Parts(parts)) => {
            for (part_index, part) in parts.iter().enumerate() {
                match part.kind.as_str() {
                    ...
                    other => warnings.push(crate::warning::Warning::new(
    ...
    if let Some(calls) = &message.tool_calls {
        for (call_index, call) in calls.iter().enumerate() {
            ...
            let id = call
                ...
                .unwrap_or_else(|| bifrost_core::tool_call_id(None, call_index));
...
fn cache_breakpoint(part: &ContentPart) -> Option<bifrost_core::CacheControl> {
    part.cache_control.as_ref().map(bifrost_core::CacheControl::from_json)
}
