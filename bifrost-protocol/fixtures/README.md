...
two layers (protocol decode, then `cc/1.53.1` encode); the `sse` fixtures pin the
response side, which it reaches through two more (wire decode, then protocol
render). One diverging byte in any of the four layers fails a test.
...
```sh
cd bifrost-protocol/fixtures/generators
...
does for all three. That is why a fixture records what the original actually does
rather than what it is believed to do: the code under test is the original's.
...
- **Item ids.** The original mints a UUID per output item, so two runs of the same
  stream differ. Bifrost names an item by the index it is reported at, and the
